...
func Test_helmUpgrade(t *testing.T) {
	is := assert.New(t)
	...
	// upgrade a single chart.  We expect to get these values back
	options := options.UpgradeOptions{Name: "nginx-1", Chart: "nginx", Namespace: "default", Repo: "https://charts.bitnami.com/bitnami"}
	optdata, err := json.Marshal(options)
	is.NoError(err)

	t.Run("helmUpgrade succeeds with admin user", func(t *testing.T) {
